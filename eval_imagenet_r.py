...
imagenet_val_location = "/data/public/imagenet2012/val/"
...
imagenet_r = dset.ImageFolder(root="/HOME/scz1973/run/wenzhao/dataset/imagenet-r", transform=test_transform)
imagenet_r_loader = torch.utils.data.DataLoader(imagenet_r, batch_size=512, shuffle=False,
...
imagenet_200_folder = "/HOME/scz1973/run/wenzhao/dataset/imagenet_val_for_imagenet_r/"
...
iid_loader = torch.utils.data.DataLoader(iid_examples, batch_size=512, shuffle=False,
                                         num_workers=8, pin_memory=True)
...
def get_predictions(loader):
    ...
    with torch.no_grad():
        for data, target in loader:
            ...
            if args.adv:
                output = model(data, target)[0][:,imagenet_r_mask]
            else:
                output = model(data)[:,imagenet_r_mask]
